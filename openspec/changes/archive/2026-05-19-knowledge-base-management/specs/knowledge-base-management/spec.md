...
- **THEN** 前端先将文件上传至 OSS 获取 URL，然后调用 `POST /api/ai/knowledge/import` 提交给后端进行 ETL 解析，并在列表中展示“解析中”状态。
...
- **THEN** 系统返回按 `created_at` 降序排列的文档分页数据，包含总记录数、总页数和当前页的数据。
...
- **THEN** 系统返回该文档的当前状态（PROCESSING / READY / FAILED）、chunk_count 以及 error_message（如果失败）。
